...
-- Design Ports Information
-- Sec	=>  Location: PIN_E8,	 I/O Standard: 2.5 V,	 Current Strength: Default
...
\Sec~output\ : fiftyfivenm_io_obuf
-- pragma translate_off
GENERIC MAP (
	bus_hold => "false",
...
-- Equation(s):
-- \S0~0_combout\ = (\d~input_o\) # ((\S0~q\ & !\S2~q\))
...
-- Equation(s):
-- \S1~0_combout\ = (\d~input_o\ & ((\S1~q\) # (!\S0~q\)))
...
-- Equation(s):
-- \S3~0_combout\ = (\S3~q\) # ((\d~input_o\ & \S2~q\))
